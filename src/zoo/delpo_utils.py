...
def setup(dataset, root, n_ways, k_shots, q_shots, order, inner_lr, device, download, task_adapt, task_adapt_fn, args):
    if dataset == 'omniglot':
        ...
        learner = CCVAE(in_channels=3, base_channels=32,
                        n_ways=n_ways, dataset='miniimagenet', task_adapt=task_adapt, task_adapt_fn=task_adapt_fn, args=args, latent_dim_l=args.zl, latent_dim_s=args.zs)
        ...
        learner = CCVAE(in_channels=3, base_channels=32,
                        n_ways=n_ways, dataset='tiered', task_adapt=task_adapt, task_adapt_fn=task_adapt_fn, args=args, latent_dim_l=args.zl, latent_dim_s=args.zs)
